...
def install():
    ...
    try:
        ...
        subprocess.run(["wget", "https://fem-on-colab.github.io/releases/fenics-install-release-real.sh", "-O", "/tmp/fenics-install.sh"], check=True)
    ...
    try:
        ...
        subprocess.run(["wget", "https://fem-on-colab.github.io/releases/gmsh-install.sh", "-O", "/tmp/gmsh-install.sh"], check=True)
        subprocess.run(["bash", "/tmp/gmsh-install.sh"], check=True)
        import gmsh
    clear_output()
...
class Wrapper(Function):
    ...
    def __call__(self, *args, **kwargs):
        if isinstance(args[0], list) or isinstance(args[0], np.ndarray):
            aaa = []
            for i in args[0]:
                aaa.append(float(super().__call__(i)))
            return aaa
        else:
    ...
    try:
        ...
        if(isinstance(obj, dolfin.cpp.mesh.Mesh)):
            ...
            if(obj.function_space().mesh().geometry().dim() != 1):
              ...
              line1, = plt.plot(xplot, obj(xplot), **{key: kwargs[key] for key in kwargs if key != "marker"})
...
def assemble(F):
    """
    Assemble discrete linear functional or bilinear form.
    Input:
        F : ufl.form.Form
            Symbolic variational form already constructed, typically obtained by evaluating
            a user-defined function with test/trial functions, e.g.:
                - F = bilinear_form(u, v)
                - F = linear_form(v)

    Output:
        - If F is a bilinear form: returns a scipy.sparse.csr_matrix
        - If F is a linear functional: returns a numpy.ndarray
        In both cases, the result is the assembled discrete representation of the form.
    """
    from fenics import assemble as assmb
    A = assmb(F)
    try:
        clear_output()
...
def L2error(uex, ufem, domain, hquad = 0.05, rquad = 4):
    ...
    uh = trial_function(Vnew)
    vh = test_function(Vnew)
    def m(u, v):
        return u * v * dx
    M = assemble(m(uh,vh))
    clear_output()
